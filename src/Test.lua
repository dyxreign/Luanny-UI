--[[
 ...
 |______| \____//_/    \_\_| \_|_| \_|     |_|     \____/|_____|
  ...
  >> LUANNY INTERFACE SUITE (v2.3)
  >> Base Original + Themes + ColorPicker
--]]
...
function TabClass:ColorPicker(options)
    ...
    expandArea.Size = UDim2.new(1, -20, 0, 100)
    ...
    local function createColorSlider(labelTxt, yPos, colorRGB)
        ...
        lbl.Text = labelTxt
        lbl.TextColor3 = theme.Text
        lbl.FontFace = FontTitle
        lbl.TextSize = 13
        
        local track = Instance.new("TextButton", expandArea)
        track.Size = UDim2.new(1, -25, 0, 8)
        track.Position = UDim2.new(0, 25, 0, yPos + 4)
        ...
        fill.BackgroundColor3 = colorRGB
    ...
    local function handleSlider(track, colorKey)
        ...
        track.InputBegan:Connect(function(input) 
            if not lockOverlay.Visible and input.UserInputType == Enum.UserInputType.MouseButton1 then sliding = true end 
        end)
        UserInputService.InputChanged:Connect(function(input)
            if sliding and input.UserInputType == Enum.UserInputType.MouseMovement then
    ...
    handleSlider(rTrack, "R")
    handleSlider(gTrack, "G")
    handleSlider(bTrack, "B")
    ...
    colorBtn.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(card, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Size = UDim2.new(0, 340, 0, isOpen and 175 or 60)
        }):Play()
    end)
    ...
    return {
        ...
        Destroy = function() card:Destroy() end,
        Set = function(v) currentColor = v; updateVisuals() end,
