...
function TabClass:ColorPicker(options)
    ...
    hueSlider.InputBegan:Connect(function(input)
        ...
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            hueSliding = true
            local pctX = math.clamp((input.Position.X - hueSlider.AbsolutePosition.X) / hueSlider.AbsoluteSize.X, 0, 1)
            H = 1 - pctX
            updateVisuals(true)
        end
    end)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement then
            if svSliding then
                ...
                local pctX = math.clamp((mx - svMap.AbsolutePosition.X) / svMap.AbsoluteSize.X, 0, 1)
                local pctY = math.clamp((my - svMap.AbsolutePosition.Y) / svMap.AbsoluteSize.Y, 0, 1)
                S = pctX
                V = 1 - pctY
                updateVisuals(true)
            elseif hueSliding then
                local mx = UserInputService:GetMouseLocation().X
                local pctX = math.clamp((mx - hueSlider.AbsolutePosition.X) / hueSlider.AbsoluteSize.X, 0, 1)
                H = 1 - pctX
                updateVisuals(true)
            end
        end
    ...
    return {
        ...
        Destroy = function() card:Destroy() end,
        Set = function(v) currentColor = v; H, S, V = currentColor:ToHSV(); updateVisuals() end,
