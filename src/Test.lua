...
function TabClass:ColorPicker(options)
    ...
    -- Expand Area Container (Começa invisível para não vazar strokes)
    ...
    -- Eventos de Clique (MouseButton1Down para ser responsivo instantaneamente)
    local svSliding = false
    svMap.MouseButton1Down:Connect(function()
        if lockOverlay.Visible then return end
        svSliding = true
        local mx, my = UserInputService:GetMouseLocation().X, UserInputService:GetMouseLocation().Y
        S = math.clamp((mx - svMap.AbsolutePosition.X) / svMap.AbsoluteSize.X, 0, 1)
        V = 1 - math.clamp((my - svMap.AbsolutePosition.Y) / svMap.AbsoluteSize.Y, 0, 1)
        updateVisuals(true)
    end)
    ...
    hueSlider.MouseButton1Down:Connect(function()
        ...
        H = 1 - math.clamp((mx - hueSlider.AbsolutePosition.X) / hueSlider.AbsoluteSize.X, 0, 1)
        updateVisuals(true)
    end)
    ...
    colorBtn.MouseButton1Click:Connect(function()
        ...
        -- Resolve o problema da UIStroke vazando pelo ClipsDescendants
        if isOpen then
